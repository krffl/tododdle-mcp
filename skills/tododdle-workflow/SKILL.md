...
- When choosing from a shared project queue, use `list_available_work` for a human-readable preview or `claim_next_task` for atomic selection. When beginning substantive execution on a known task, use `claim_task` with a stable opaque run ID for the current agent run. Use a unique run ID per claim-next operation, call `renew_task_claim` before its lease expires, use `WAITING` while paused for a short dependency or reply, and call `release_task` when execution stops, completes, or hands off. Claims identify the authenticated Agent Connection by its configured label and never replace the human assignee. Do not claim tasks for simple reads or planning-only discussion.
